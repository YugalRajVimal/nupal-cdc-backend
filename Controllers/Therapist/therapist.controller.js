...
//   // Supports: search (Children name, Children id, appointmentId), pagination (?page, ?limit)
...
//           // Check Children name
//           let patientName = "";
//           if (sess.Children && sess.patient.name) {
//             patientName = sess.patient.name.toLowerCase();
//           }
//           // Children ID
//           let patientId = "";
//           if (sess.Children && sess.patient.patientId) {
...
class TherapistController {
  ...
  async getAllTherapistAppointments(req, res) {
    try {
      ...
      // Use therapist profile's id to find appointments/bookings
      // Fetch appointments but do not populate or return therapist or payment info
      const appointmentsRaw = await Booking.find({ therapist: therapistProfile._id })
        .populate({
          path: "package",
          model: "Package",
          select: "-costPerSession -totalCost"
      ...
      // ── CHANGED: rename patient → Children in each appointment response ──
      const appointments = appointmentsRaw.map(app => {
        ...
        // Rename patient → Children at the top level
        if ('patient' in clean) {
          clean.Children = clean.patient;
  ...
  // Fetch all bookings, then for all sessions, match with therapist, and respond with appointmentId, Children, therapyType, this therapist's session details
  // Supports: search (Children name, Children id, appointmentId), pagination (?page, ?limit)
  async getAllTherapistSessions(req, res) {
    try {
      const therapistId = req.user.id;
        ...
        if (Array.isArray(booking.sessions)) {
          booking.sessions.forEach(session => {
            if (session.therapist && String(session.therapist) === String(therapistProfile._id)) {
              ...
              // Remove _id field if present
              if ('_id' in sessionCopy) delete sessionCopy._id;
              // ── CHANGED: key renamed from patient → Children ──
              therapistSessions.push({
                appointmentId: booking.appointmentId || booking._id,
                Children: booking.patient,   // ── CHANGED from patient ──
                therapyType: booking.therapy,
                session: sessionCopy,
              });
            }
      ...
      if (search.length > 0) {
        filteredSessions = therapistSessions.filter(sess => {
          // ── CHANGED: reference Children instead of patient ──
          // Check Children name
          let childrenName = "";
          if (sess.Children && sess.Children.name) {
            childrenName = sess.Children.name.toLowerCase();
          }
          // Children ID
          let childrenId = "";
          if (sess.Children && sess.Children.patientId) {
            childrenId = (sess.Children.patientId + "").toLowerCase();
          }
          // Appointment ID (as string, in case someone searches by it)
          let appointmentId = (sess.appointmentId ? sess.appointmentId + "" : "").toLowerCase();
          return (
            childrenName.includes(search) ||
            childrenId.includes(search) ||
            appointmentId.includes(search)
          );
        });
      }
          ...
          appointment.sessions.forEach(session => {
            ...
            if (
              ...
              allSessions.push({
                ...session,
                appointmentId: appointment.appointmentId,
                Children: appointment.patient,    // ── CHANGED from patient ──
                therapist: appointment.therapist,
                therapyType: appointment.therapy,
              });
            }
