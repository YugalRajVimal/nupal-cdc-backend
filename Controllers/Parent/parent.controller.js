...
class ParentController {
  ...
  async parentSignUpSendOTP(req, res) {
    ...
    try {
      ...
      // --- Send OTP to both email and WhatsApp (if present), like AuthController ---
      ...
      if (whatsappError) {
        ...
        console.error("ParentSignUp OTP WhatsApp error:", whatsappError);
      }

      let whatsappStatus = null;
      if (phoneToSave) {
        whatsappStatus = whatsappError ? (whatsappError?.message || "Failed to send OTP on WhatsApp") : "SENT";
      } else {
      ...
      return res.json({
        success: true,
        message: "OTP sent to email address." + (phoneToSave
          ? (whatsappError ? " (Email sent, but WhatsApp failed)" : " (Also sent on WhatsApp)")
          : ""),
  ...
  async parentSignUpResendOTP(req, res) {
    const session = await User.startSession();
    session.startTransaction();
    ...
    try {
      ...
      // Find existing user w/ pending signup as parent (role: "patient")
      const user = await User.findOne({ email, role: "patient" }).session(session);
      ...
      // Only allow resend if user has not completed signup
      // if (user.status === "active") {
      //   await session.abortTransaction();
      //   session.endSession();
      //   return res.status(400).json({ success: false, message: "Signup already completed." });
      // }
      ...
      const expiresInMs = 1000 * 300;
      user.signUpOTP = otp;
      user.signUpOTPExpiresAt = new Date(Date.now() + expiresInMs);
      user.signUpOTPSentAt = new Date();
      user.signUpOTPAttempts = 0;

      await user.save({ session });

      // Send OTP via Email and WhatsApp if available
      const phone = user.phone;
      if (email) {
        // You may use the same sendMail utility as in signup, adjust as per your codebase.
        sendEmailPromise = sendMail(
          email,
          "Your OTP Code",
          `Your OTP is: ${otp}`
        ).catch((err) => { sendEmailError = err; });
      ...
      if (sendEmailError) {
        await session.abortTransaction();
        session.endSession();
        console.error("Error: Failed to send OTP to email address (parent resend).", sendEmailError);
        return res.status(500).json({
          success: false,
          message: "Failed to send OTP to email address.",
          emailError: sendEmailError?.message || sendEmailError
      ...
      // If WhatsApp failed, log soft fail but do not block resend
      if (whatsappError) {
        console.error("ParentSignUpResend OTP WhatsApp error:", whatsappError);
      }
      ...
      console.log(`[ParentSignup-Resend] OTP for ${email}:`, otp);
      ...
      try {
        await AuditLogService.addLog({
          action: 'PARENT_SIGNUP_OTP_RESENT',
          user: user._id,
          role: 'parent',
          resource: 'Parent',
          resourceId: user._id,
          details: {
            ...
            name: user.name || null,
            phone: phone || null,
            message: `Parent signup OTP resent to ${email}`,
            context: 'Parent self-signup OTP re-sent',
            whatsappStatus
          },
          ipAddress: req.ip,
          userAgent: req.headers['user-agent']
        ...
        console.error('Failed to write audit log (PARENT_SIGNUP_OTP_RESENT) in parentSignUpResendOTP:', logErr);
      }

      return res.json({
        success: true,
        message: "OTP resent to email address." + (phone
          ? (whatsappError ? " (Email sent, but WhatsApp failed)" : " (Also sent on WhatsApp)")
          : ""),
        whatsappStatus
      });
      ...
      await session.abortTransaction();
      session.endSession();
      console.error("Error in parentSignUpResendOTP:", e);
      return res.status(500).json({ success: false, message: "Server error." });
    }
  }
