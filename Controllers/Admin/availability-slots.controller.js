...
class AavailabilitySlotsAdminController {
  ...
  async getAvailabilitySummary(req, res) {
    try {
      ...
      // ── KEY FIX ────────────────────────────────────────────────────────────
      // When called from updateBooking, req.query.excludeBookingId contains
      // the _id of the booking being edited.  We exclude ALL sessions from
      // that booking when building the booked-slots map, so those slots are
      // NOT reported as "already booked" — they belong to the booking being
      // edited, not to any other booking.
      ...
      // { _id: { $ne: excludeBookingId } } so the booking being edited is
      // completely absent from the booked-slots aggregation.
      ...
      for (const bk of bookings) {
        ...
        for (const sess of bk.sessions) {
          ...
          // ── FIX ──────────────────────────────────────────────────────────
          // A session marked 'Missed' gave up its slot — the patient never
          // showed, so that date/time/therapist combination must be free
          // for rebooking (as a replacement session on this booking, or as
          // any other booking entirely). Without this skip, a missed
          // session permanently locks its slot even though nobody is
          // actually occupying it.
          if (sess.status === 'Missed') continue;
          // ─────────────────────────────────────────────────────────────────
      ...
      for (const { display, iso } of dateKeys) {
        ...
        if (filterTherapistId) {
          availableTherapists = availableTherapists.filter(
        ...
        const totalLimitedSlots = availableTherapists.length *
          SESSION_TIME_OPTIONS.filter(s => s.limited).length;
  ...
  getMonthlyAvailabilitySummary = async (req, res) => {
    try {
      ...
      if (
        !month || !year || isNaN(month) || isNaN(year) ||
        month < 1 || month > 12
      ) {
      ...
      const reqMock = { query: { from, to } };
      if (therapistId) reqMock.query.therapistId = therapistId;
      // Note: excludeBookingId is NOT passed here because
      // getMonthlyAvailabilitySummary is a general calendar view,
      // not an edit flow — all bookings should be counted.
