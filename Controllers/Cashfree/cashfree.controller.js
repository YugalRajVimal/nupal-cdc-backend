...
CashfreePG.XClientId = process.env.CASHFREE_CLIENT_ID;
CashfreePG.XClientSecret = process.env.CASHFREE_CLIENT_SECRET;
CashfreePG.XEnvironment =
  process.env.NODE_ENV === "development"
    ? CashfreePG.Environment.SANDBOX
    : CashfreePG.Environment.PRODUCTION;
...
class CashfreeController {
  ...
  generateSessionId = async (req, res) => {
    ...
    try {
      ...
      if (response?.data) {
        ...
        // FIX: mark the payment method as cashfree as soon as an order is created,
        // so it's already correct even before confirmStatus/webhook runs.
      ...
      if (payment && data?.order_status) {
        switch (data.order_status) {
          case 'PAID': {
            ...
            payment.amountPaid = data.order_amount || payment.amountPaid || 0;
            payment.paymentTime = data.payment_time ? new Date(data.payment_time) : new Date();
            payment.paymentMethod = 'cashfree'; // FIX: was never set on the Payment doc

            // Gather childrenName, childrenId, description, and the booking link
            // NOTE: Cashfree's response has no concept of childrenName/childrenId/booking —
            // those are our own business fields, so we always resolve them from our own
            // Booking collection, not from `data`.
            let childrenName = payment.childrenName || undefined;
            let childrenId = payment.childrenId || undefined;
            let description = payment.remark || `Cashfree Payment for order ${order_id}`;
            let linkedBooking = null; // FIX: needed to attach booking._id to the Finance record

            try {
              const booking = await Booking.findOne({ payment: payment._id }).populate('patient');
              if (booking) {
                linkedBooking = booking; // FIX
                if (booking.patient && booking.patient.name) {
                  childrenName = booking.patient.name;
                  childrenId = String(booking.patient.patientId || booking.patient._id);
                }
                ...
                console.warn(`[confirmStatus] No Booking found linked to payment ${payment._id} (order ${order_id}) — childrenName/childrenId/booking will be blank on the Finance record.`);
              }
            } catch (e) {
              console.error("[confirmStatus] Error resolving booking/patient for finance record:", e);
            }
            ...
            financeRecord = await Finances.create({
              ...
              childrenName,
              childrenId,
              booking: linkedBooking ? linkedBooking._id : undefined // FIX: was missing
            });
            break;
          }
          case 'FAILED':
      ...
      return res.status(200).json({
        ...
        financeRecord: financeRecord
          ? {
            ...
            date: financeRecord.date,
            paymentMethod: financeRecord.paymentMethod,
            childrenName: financeRecord.childrenName,
            childrenId: financeRecord.childrenId,
            booking: financeRecord.booking
          }
          : null
      });
    } catch (error) {
  ...
  handleWebhook = async (req, res) => {
    ...
    try {
      ...
      const Finances = (await import('../../Schema/finances.schema.js')).default; // FIX: needed for finance creation below
      ...
      if (payment_status === "SUCCESS") {
        ...
        payment.amountPaid = transaction_amount;
        payment.paymentTime = payment_time;
        payment.paymentMethod = "cashfree"; // FIX: was never set here either

        // Link back to booking and resolve childrenName/childrenId, same as confirmStatus
        const booking = await Booking.findOne({ payment: payment._id }).populate('patient');
        ...
        if (booking) {
          ...
          console.warn(`[handleWebhook] No Booking found linked to payment ${payment._id} (order ${order_id}).`);
        }

        await payment.save();

        // FIX: handleWebhook previously never created a Finances record at all on SUCCESS.
        // Guard against double-counting: only create it if confirmStatus hasn't already
        // created one for this order (adjust this check to whatever uniqueness you prefer,
        // e.g. checking Finances by utr/order_id before creating).
        const existingFinance = await Finances.findOne({ utr: order_id });
        if (!existingFinance) {
